...
twitter <- read.csv("twitter.csv",stringsAsFactors=FALSE)   #ALTERNATIVELY: read.csv("twitter.csv",stringsAsFactors=FALSE) 
...
#dm = data.frame(word=colnames(twittersparse), freq=word_freqs)
# Plot wordcloud
...
model1 <- glm(data=train, family=binomial, Neg~.)
summary(model1)
#
# Let's now try to make a prediction on the test data
predict1 <- predict(newdata = test, model1,type="response")
# How does the model perform?
table1 <- table(predict1>=0.5,test$Neg)
table1
...
# Note that, although we have removed words such as "drive", "driving", and "driver" using the tm_map function,
# in the final corpus words such as "drives" still exist.
...
#
# Build the model
model2 <- rpart(data = train, Neg~., method = "class")
summary(model2)
# Tree visualization
prp(model2) 
# Prediction
predict2 <- predict(model2, newdata = test, type = "class")
table2 <- table(predict2,test$Neg)
table2
...
model2a <-  rpart(data = train, Neg~., method = "class", cp= 10^-6)
prp(model2a,type=4,extra=4) 
printcp(model2a)
plotcp(model2a)
# Now, let's try to prune it
model2b <- prune(model2a,cp=0.004059) # To prune, we use the smallest value of xerror
...
summary(model3)
# Prediction
predict3 <- predict(model3, newdata = train, type = "class")
table3 <- table(predict3,train$Neg)
table3
sum(diag(table3)/sum(table3))
# predict3 FALSE TRUE
# FALSE  1306   99
# TRUE      16  443
# The accuracy is (1306+443)/(1864) = 0.93


# Prediction (test dataset) 
predict3 <- predict(model3, newdata = test, type = "class")
table3 <- table(predict3,test$Neg)
table3
sum(diag(table3)/sum(table3))
#predict3 FALSE TRUE
#FALSE   502  152
